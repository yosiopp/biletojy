...
インデックス: `tickets_at_idx (created_at)`, `tickets_by_idx (created_by)`
...
| tag | VARCHAR(255) | NOT NULL UNIQUE | タグ名（例: `status:OPEN`, `due-date@:`） |
| note | VARCHAR(255) | | 説明 |
| color | VARCHAR(40) | | 表示色（例: `#e11d48`） |
| is_group | INTEGER | NOT NULL DEFAULT 0 | `:` を含むタググループなら1 |
| is_range | INTEGER | NOT NULL DEFAULT 0 | グループ名末尾が `@` の日時タグなら1 |
