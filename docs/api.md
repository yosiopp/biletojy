...
* 作成・編集時に `tags` のうちタグカタログ未定義のタグは自動登録される
  ...
  * `sort_order` には同一グループ内の最大値 + 1 が設定され、グループ末尾に並ぶ
    （グループでないタグ同士はまとめてひとつの並びとして扱い、一覧の末尾に登録順で並ぶ）
...
* 一覧はタググループ（`status:` などの接頭辞）を接頭辞順でまとめ、グループでないタグは全グループの後にまとめて返す。
  それぞれの中は `sort_order` 昇順（同値はタグ名昇順）。タググループのプルダウンの選択肢はこの順に並ぶ
...
タググループ内の選択肢や、グループでないタグ同士の並び替えに使う
（同じ並び替え単位——同一グループ、またはグループでないタグ全体——のタグIDをまとめて送る）。
