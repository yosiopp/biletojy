...
リクエストボディは1MiBまで（添付ファイルのアップロードは10MiBまで）で、超えた場合は `413 Request Entity Too Large` を返す。
...
| `GET /api/images/{id}` | 添付ファイル配信（旧URLの後方互換エイリアス） |
...
### 旧URL（GET /api/images/{id}）
ファイル一般化前にアップロードされた画像を参照している既存チケット本文（`/api/images/{id}` 形式のリンク）のための
後方互換エイリアスで、`GET /api/files/{id}` と同じ内容を返す。旧 `POST /api/images` は `POST /api/files` に置き換えられ廃止。
