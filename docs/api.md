...
| `GET /api/tickets/export?q=&tags=&format=` | チケットのエクスポート（JSON / markdown） |
| `POST /api/tickets/import` | エクスポートデータのインポート |
| `GET /api/files` | 添付ファイル一覧 |
| `POST /api/files` | 添付ファイルアップロード |
| `GET /api/files/{id}` | 添付ファイル配信 |
| `DELETE /api/files/{id}` | 添付ファイル削除 |
...
| `POST /api/tags` | タグ作成 |
| `GET /api/tags/export` | タグカタログのエクスポート |
| `POST /api/tags/import` | タグカタログのインポート |
| `POST /api/tags/restore-defaults` | デフォルトタグの復元 |
| `PUT /api/tags/{id}` | タグ編集 |
| `PUT /api/tags/{id}/rename` | タグ名の変更（使用中チケットの一括書き換え） |
| `GET /api/tags/{id}/usage` | タグを使用しているチケット数 |
...
* `tags` — タグ条件のカンマ区切り。すべての条件を満たすチケットに絞り込む（AND条件）
  * 完全一致に加え、階層タグ（`/` 区切り）は前方一致で子孫タグにもマッチする。
    この前方一致はグループなしタグに限らずすべてのタグに適用される
    （例: `status:OPEN` はグループ値を階層化した `status:OPEN/x` にもマッチする）
...
* 存在しないチケットIDを指定した場合は `404 Not Found` を返す（サブリソースGET共通）
...
* 作成・編集時に `tags` の各タグ（スペース区切り）はタグカタログAPIと同じ検証を受け、
  検索構文のメタ文字と衝突するタグ（`,`・`|` を含む、または先頭が `-`）を含む場合は `400 Bad Request` を返す。
  既存データ互換のため、編集時にそのチケットへ既に付いているタグは検証せず（検証導入前のタグを持つ
  チケットも編集・履歴復元できる）、検索・取得など読み取り側もこの検証を行わない
...
* 一覧（`GET /api/tickets/{id}/comments`）・追加とも、存在しないチケットIDを指定した場合は `404 Not Found` を返す

## 履歴
チケット・コメントの作成・編集のたびに履歴テーブルへ保存された各版の内容を返す。
古い版から順（作成時の版 → 編集後の版）に返し、存在しないチケット・コメントのIDを指定した場合は
`404 Not Found` を返す（サブリソースGET共通）。
...
### エクスポート（GET /api/tickets/export?q=&tags=&format=）
...
* バックアップの復元用のため `tags` の検証は行わない（検証導入前のDBからのエクスポートもそのまま取り込める）。
  タグカタログAPIの検証に通らないタグはカタログへ自動登録されない
...
### 一覧（GET /api/files）
アップロード済みの全ファイルをID降順（新しい順）で返す。バイナリ本体は含めない。

```
FileInfo {
  id: number
  name: string               // アップロード時のファイル名（未指定は空文字）
  mime: string
  size: number               // バイト数（LENGTH(data)）
...
参照の有無は、本文中の `/api/files/{id}` のmarkdownリンクをタグ名変更（rename）と同じ2段構えで判定する:
LIKEで `/api/files/` を含む本文に候補を絞った上で、IDの直後が数字でないことをサーバー側で確認する
（`/api/files/1` の判定が `/api/files/12` に誤マッチしない）。`referenced` は現役のチケット・コメント本文、
`history_referenced` はチケット・コメントの履歴（`ticket_histories` / `comment_histories`）が対象で、
「履歴のみからの参照」は `!referenced && history_referenced` で判別できる
（作成・編集のたびに履歴へ版が保存されるため、現役から参照されているファイルは通常、履歴からも参照されている）。

### 削除（DELETE /api/files/{id}）
ファイルを削除し、`204 No Content` を返す。存在しないファイルの削除は `404 Not Found` を返す。
チケット・コメントからの参照が残っていても削除できる（本文のリンクは書き換えず、リンク切れになる。
参照有無に応じた警告はUI側で行う）。
...
Tag {
  ...
  sort_order: number   // セクション内の表示順（作成時に同一セクションの末尾へ自動採番。並び替えAPIで振り直し）
}
```
...
* `sort_order` は作成時に同一セクション（グループ / 値なしのグループエントリ全体 / グループでないタグ全体）の
  最大値 + 1 が自動設定され、一覧・プルダウンでセクション末尾に並ぶ。編集では変更されない（並び替えAPIで振り直せる）
...
### 使用数（GET /api/tags/{id}/usage）
タグを使用しているチケット数を返す（削除・タグ名変更の確認ダイアログ用。チケット本体は返さない）。

```
{ "count": 3 }
```
* 使用の判定はタグ単位の完全一致（チケット検索の前方一致や日時・数値の範囲解釈はしない）
  * 値なしのグループエントリ（`due-date@:` など末尾 `:`）は、そのグループの値を持つチケットを前方一致で数える
  * 階層タグの子孫は別タグのため数えない（`docs` の使用数に `docs/design` は含まれない）
* 存在しないタグIDを指定した場合は `404 Not Found` を返す
...
### エクスポート（GET /api/tags/export）
タグカタログ全件を、別環境への複製・バックアップ・再インポート用のJSONとして返す。
`Content-Disposition: attachment` 付きで `biletojy-tags.json` としてダウンロードさせる。
`id` と導出属性（`is_group` / `is_range`）は含めず、`tag` / `note` / `color` / `sort_order` のみを返す。

```
{ "tags": [ { "tag": "status:OPEN", "note": "未処理", "color": "#e11d48", "sort_order": 1 }, ... ] }
```

### インポート（POST /api/tags/import）
エクスポートと同じ形式の `tags` 配列を受け取り、タグカタログへ一括登録する。

```
{ "tags": [ { "tag": "priority:HIGH", "note": "優先度", "color": "#ff0000", "sort_order": 9 }, ... ] }
```
* 各タグの `tag` はタグ作成（`POST /api/tags`）と同じ検証を受け、違反する名前を含む場合は `400 Bad Request` を返す
* `tags` が空または未指定の場合は `400 Bad Request` を返す
* `is_group` / `is_range` は指定によらずタグ名から導出される
* 同名の既存タグは変更せずスキップする（衝突しても既存タグは壊さない）
* `sort_order` は正の値ならそのまま尊重し（エクスポート→インポートの往復で並び順を再現できる）、
  `0` または未指定のときは同一セクション（グループ / 値なしのグループエントリ全体 / グループでないタグ全体）の
  末尾へ採番する
* 全件をひとつのトランザクションで登録する（全件成功か全件失敗のどちらか）
* 成功時は `201 Created` で `{"imported": 登録件数, "skipped": スキップ件数}` を返す

### デフォルトタグの復元（POST /api/tags/restore-defaults）
初期シードと同じデフォルトのタグ定義（`status:*` / `type:*` / `due-date@:`）のうち、
カタログに存在しないものだけを追加する。既存のタグ（名前・色・並び順のカスタマイズ）は変更しない。
* ボディは不要
* 成功時は `200 OK` で `{"restored": 追加件数}` を返す（不足がなければ `0`）
